...
export default function Home() {
  ...
  const handleAddSubject = () => {
    if (subject.trim() !== "") {
      ...
      setStudyGuide([...studyGuide, { subject: subject.trim(), information: [] }]);
      setSubject("");
    }
  };
  ...
  const handleCreateStudyGuide = async () => {
    ...
    const updatedStudyGuide = await Promise.all(subjectList.map(async (subject) => {
      const apiOutput = await callAPI(subject);
      if (apiOutput) {
        // Split the API output by "." and add each part to the study guide
        return {
          subject,
          information: apiOutput.split('.').map((info, index) => `${info.trim()} (Test ${index + 1})`)
        };
      } else {
  ...
  const generatePDF = () => {
    ...
    studyGuide.forEach(item => {
      // Add main subject
      if (yPos > 250) { // If content exceeds page height, create new page
        doc.addPage();
        yPos = 20;
        page++;
      }
      ...
      item.information.forEach(info => {
        ...
        doc.setFontSize(12);
        const textWidth = doc.getStringUnitWidth(`${info}`) * 12; // Calculate width of text
        const lineWidth = 190 - textWidth; // Calculate width of line
        doc.text(`${info}`, 20, yPos); // Add information
    ...
    <>
      ...
      >
        <div className="flex items-center mb-4">
          <button className="bg-[#FDE8C4] hover:bg-[#fce0ae] text-gray-800 font-bold py-2 px-4 rounded-full shadow-md text-now flex items-center" onClick={toggleDropdown}>
            <img src="down2.svg" className="h-6 w-6 ml-2 mr-2" alt="Dropdown Icon" />
            <span className="text-sm">Input Format: {type}</span>
          </button>
          {isOpen && (
            <div className="absolute bg-white border rounded mt-2">
              <a href="#" className="block px-4 py-2 text-gray-800 hover:bg-gray-200 shadow-md" onClick={() => handleItemClick("Photo")}>Photo</a>
              <a href="#" className="block px-4 py-2 text-gray-800 hover:bg-gray-200 shadow-md" onClick={() => handleItemClick("Text")}>Text</a>
            </div>
          )}
          <button className="bg-[#FDE8C4] hover:bg-[#fce0ae] text-gray-800 font-bold py-2 px-4 rounded-full shadow-md text-now flex items-center ml-auto" onClick={handleCreateStudyGuide}>
            Create Study Guide
          </button>
          <button className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 ml-4 rounded-full shadow-md" onClick={handleReset}>
            Reset
          </button>
          <button className="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 ml-4 rounded-full shadow-md" onClick={generatePDF}>
            Export Study Guide
          </button>
        </div>

        <div className="flex items-center mt-4">
          <label className="text-gray-800 font-bold mr-2">Study Guide Title:</label>
          <input
...

        {
          type === "Photo" ? (
            <div>
              <input type="file" accept="image/*" style={{ marginTop: '10px' }} multiple />
            </div>
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', marginTop: '10px' }}>
              ...
              />
              <button className="bg-[#FDE8C4] hover:bg-[#fce0ae] text-gray-800 font-bold py-2 px-4 rounded-full shadow-md text-now flex items-center" onClick={handleAddSubject}>
                <img src="plus.svg" className="h-6 w-6 ml-2 mr-2" alt="Plus Icon" />
              </button>
            </div>
          )
        }
        ...
        <div ref={studyGuideRef}>
          <p className="text-gray-800 font-bold mb-2">
            ...
            <br />
            To add more options under each subject, simply type in the input box and press Enter.
          </p>
          {studyGuide.map((item, subjectIndex) => (
            <div key={subjectIndex} className="flex flex-col items-start mt-2">
              <div className="bg-transparent p-2 rounded-md cursor-pointer" onClick={() => handleRemoveSubjectWithSentences(subjectIndex)}>
                {item.subject}
              </div>
              {item.information.map((sentence, sentenceIndex) => (
                <div key={sentenceIndex} className="flex items-center ml-4 cursor-pointer" onClick={() => handleRemoveSentence(subjectIndex, sentenceIndex)}>
                  <p className="mt-2 ml-2">{sentence}</p>
                </div>
              ))}
              {isGuideGenerated && ( // Only show "Add Information" button if guide is generated
                <button className="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 ml-4 rounded-full shadow-md" onClick={() => handleAddInformation(subjectIndex)}>
                  Add Information
                </button>
              )}
            </div>
